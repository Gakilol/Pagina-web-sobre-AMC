...
import { Menu, X, Cpu, BookOpen, Terminal, Download, Zap, CircuitBoard, Search, HelpCircle } from 'lucide-react'
...
const NAV_LINKS = [
  ...
  { href: '/diagramas',        label: 'Diagramas',       icon: Zap,          color: 'text-purple-400', border: 'hover:border-purple-500/50', bg: 'hover:bg-purple-500/5' },
  { href: '/glosario',         label: 'Glosario',        icon: HelpCircle,   color: 'text-rose-400',   border: 'hover:border-rose-500/50',   bg: 'hover:bg-rose-500/5' },
...
export function Header() {
  ...
  return (
    <>
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3.5 sm:px-6 lg:px-8 gap-4">
          {/* Logo */}
              ...
              return (
                <Link
                  ...
                  className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 border ${
                    isActive
                      ? `border-primary/45 bg-primary/8 ${color}`
                      : `border-transparent text-muted-foreground ${border} ${bg} hover:text-foreground`
                  }`}
                >
                  <Icon size={13} className={isActive ? color : 'opacity-70'} />
                  <span>{label}</span>
                  {isActive && (
          ...
          {/* Smaller Screen nav wrapper (medium width display support) */}
          <nav className="hidden md:flex xl:hidden items-center gap-0.5">
            {NAV_LINKS.slice(0, 4).map(({ href, label, icon: Icon, color, border, bg }) => {
              const isActive = pathname === href || pathname.startsWith(href + '/')
              return (
                <Link
                  key={href}
                  href={href}
                  className={`relative flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 border ${
                    isActive
                      ? `border-primary/45 bg-primary/8 ${color}`
                      : `border-transparent text-muted-foreground ${border} ${bg} hover:text-foreground`
                  }`}
                >
                  <Icon size={12} className={isActive ? color : 'opacity-70'} />
                  <span>{label}</span>
                </Link>
              )
            })}
            
            {/* More menu item for narrow desktop layouts */}
            <Link
              href="/glosario"
              className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold border border-transparent text-muted-foreground hover:bg-rose-500/5 hover:text-foreground`}
            >
              <HelpCircle size={12} className="opacity-70 text-rose-400" />
              <span>Glosario</span>
            </Link>
          </nav>

          {/* Search Button & Status badge */}
          <div className="hidden md:flex items-center gap-3">
            <button
              onClick={triggerSearch}
              className="flex items-center gap-2.5 px-3.5 py-1.5 rounded-lg border border-border/80 bg-card/40 hover:bg-card/75 text-xs font-mono text-muted-foreground/75 hover:text-foreground hover:border-primary/40 hover:shadow-[0_0_12px_rgba(9,137,255,0.06)] transition-all cursor-pointer select-none"
            >
              <Search size={13} className="text-muted-foreground" />
              <span>Buscar</span>
              <kbd className="bg-background border border-border/80 px-1.5 py-0.2 rounded text-[9px] text-muted-foreground/50 shadow-sm">Ctrl+K</kbd>
            </button>

            <div className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-border/40 bg-card/30 text-[9px] font-mono text-muted-foreground/60 uppercase tracking-wider select-none">
          ...
          <div className="flex md:hidden items-center gap-2">
            <button
              onClick={triggerSearch}
              className="p-2 hover:bg-accent/60 rounded-xl transition-all duration-200 border border-transparent hover:border-border/40 text-muted-foreground"
              aria-label="Buscar"
            >
              <Search size={18} />
            </button>
            ...
            <button
              className="p-2 hover:bg-accent/60 rounded-xl transition-all duration-200 border border-transparent hover:border-border/40"
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 border ${
                    isActive
                      ? 'border-primary/30 bg-primary/8 text-white animate-fade-in'
                ...
                >
                  <div className={`flex items-center justify-center w-8 h-8 rounded-lg ${isActive ? 'bg-primary/15' : 'bg-muted/40'}`}>
                    <Icon size={15} className={isActive ? color : 'opacity-60'} />
                  </div>
                  <span>{label}</span>
                  {isActive && (
